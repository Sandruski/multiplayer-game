    ...
    if (state == ServerState::Listening) {
        ...
        if (message == ClientMessage::Hello) {
            ...
            if (proxy == nullptr) {
                ...
                GameObject* playerGameObject = spawnPlayer(*proxy, spaceshipType);
        ...
        for (ClientProxy& clientProxy : clientProxies) {
            if (clientProxy.connected) {
                ...
                if (clientProxy.secondsSinceLastReplication >= replicationDeliveryIntervalSeconds
					&& clientProxy.m_replicationManager.hasPendingReplicationCommandsToWrite()) {
...
ModuleNetworkingServer::ClientProxy* ModuleNetworkingServer::getClientProxy(const sockaddr_in& clientAddress)
{
    ...
    for (int i = 0; i < MAX_CLIENTS; ++i) {
        if (clientProxies[i].address.sin_addr.S_un.S_addr == clientAddress.sin_addr.S_un.S_addr && clientProxies[i].address.sin_port == clientAddress.sin_port) {
...
GameObject* ModuleNetworkingServer::spawnPlayer(ClientProxy& clientProxy, uint8 spaceshipType)
{
    ...
    for (int i = 0; i < MAX_CLIENTS; ++i) {
        if (clientProxies[i].connected) {
            ...
            clientProxies[i].m_replicationManager.create(clientProxy.gameObject->networkId);
        }
    }
