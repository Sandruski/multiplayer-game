...
static bool collisionTest(Collider& c1, Collider& c2)
{
    ...
    mat4 aWorldMatrix = translation(a->position) * rotationZ(radiansFromDegrees(a->angle)) * scaling(asize) * translation(vec2 { 0.5f, 0.5f } - a->pivot);

    mat4 bWorldMatrix = translation(b->position) * rotationZ(radiansFromDegrees(b->angle)) * scaling(bsize) * translation(vec2 { 0.5f, 0.5f } - b->pivot);
        ...
        if (c1.type != ColliderType::None) {
            for (uint32 j = i + 1; j < activeCollidersCount; ++j) {
                ...
                if (c2.type != ColliderType::None) {
                    if ((c1.isTrigger && c1.gameObject->behaviour != nullptr) || (c2.isTrigger && c2.gameObject->behaviour != nullptr)) {
...
void Collider::write(OutputMemoryStream& packet) const
{
    packet.Write(isTrigger);
}

void Collider::read(const InputMemoryStream& packet)
{
    packet.Read(isTrigger);
}
